...
class Comment extends Component{
    ...
    _updateTimeString(){
        ...
        this.setState({
            timeString: duration > 60 ? (duration > 3600 ? (duration > 86400 ? (duration > 259200 ? this.toLocalDate(this.state.timeString) : `${Math.round(duration/3600)}天前`) : `${Math.round(duration/3600)}小时前`) : `${Math.round(duration/60)}分钟前`) : `${Math.round(Math.max(duration, 1))} 秒前`
        })
    }
